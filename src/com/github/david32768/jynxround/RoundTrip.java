...
public class RoundTrip {
    ...
    private static boolean roundTripClass(String classname, EnumSet<GlobalOption> options) {
        ...
        String disasm = disassemble(originalbytes);
        if (disasm == null) {
            System.out.format("disassembly of %s failed%n", classname);
            return false;
        }
        
        byte[] resultbytes = assemble(classname, disasm);
        if (resultbytes == null) {
            System.out.format("assembly of %s failed%n", classname);
            return false;
        }
        
        return compare(originalbytes, resultbytes);
    }
    ...
    private static String disassemble(byte[] bytes) {
        ...
        Global.pushGlobal(mainoption);
        var main = mainoption.mainOptionService();
        String result = main.callToString(bytes);
        Global.popGlobal();
        return result;        
    }
    
    private static byte[] assemble(String classname, String disasm) {
    ...
    private static boolean compare(byte[] originalbytes, byte[] resultbytes) {
        Global.pushGlobal(MainOption.COMPARE);
        var comparator = CompareClassFiles.usingASM(originalbytes, resultbytes);
        boolean ok = comparator.compareString();
        Global.popGlobal();
        return ok;
    }
}
